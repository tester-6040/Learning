...
$avgAttendance = $totalEmployees > 0
    ? round(array_sum(array_map(static fn (array $row): float => (float) ($row['attendance_percentage'] ?? 0), $summary)) / $totalEmployees, 2)
    : 0;
...
<header class="bg-slate-900 text-white shadow-lg">
    <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between gap-3">
        <div>
        ...
        <div class="text-right">
            <p class="text-sm text-slate-200">Signed in as <span class="font-semibold"><?= htmlspecialchars((string) ($user['full_name'] ?? 'Admin')) ?></span></p>
            <button id="logoutBtn" class="mt-1 bg-red-500 hover:bg-red-600 text-white text-sm px-3 py-1 rounded-md">Logout</button>
        </div>
    </div>
</header>
...
document.getElementById('logoutBtn').addEventListener('click', async () => {
    const response = await fetch(`${API_BASE}?route=auth/logout`, {method: 'POST'});
